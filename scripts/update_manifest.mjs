// Build or validate SignalSpace's updater manifest (latest.json).
//
...
//
// It rejects:
//   - a manifest missing windows-x86_64, the only platform SignalSpace ships
...
//     --pub-date 2026-08-04T00:00:00Z --notes "SignalSpace 2.6.0" \
...
/** Windows is the only platform SignalSpace builds. */
 ...
 *       ...\bundle\nsis\SignalSpaceFinance_2.6.0_x64-setup.exe.sig
 *
...
function main() {
  ...
  if (options.validate) {
    ...
    console.log(`${options.validate} is a valid SignalSpace updater manifest.`);
    return;
  }
