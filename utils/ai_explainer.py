"""
SignalSpace AI Copilot — grounded explanations, summaries, and a small Ask SignalSpace.
...
_REPAIR_SYSTEM = (
    ...
    "fences. Use only the provided SignalSpace evidence. Do not invent numbers. "
    "Keep text concise."
)
...
_COPILOT_SYSTEM = (
    ...
    "No reasoning. No <think>. Use only the provided SignalSpace evidence. "
    "Do not invent numbers. Do not give financial advice beyond the evidence. "
    "Keep text concise.\n\n"
    "You are SignalSpace's local financial copilot. Explain the user's state using "
    ...
    if packet.get("data_confidence", {}).get("score", 0) == 0:
        ...
        return {
            ...
            "summary":  "Import at least one statement to get started. "
                        "Everything SignalSpace shows is computed locally from imported data.",
            "moves":    [
...
# ── 5. Ask SignalSpace — skill router ──────────────────────────────────────────
...
ASK_PRESETS = [
    ...
    # Pass 22 — Ask SignalSpace v3 presets aligned to the planning loop.
...
def _build_ask_packet(skill: str, conn: sqlite3.Connection) -> dict:
    ...
    # ── Pass 22: Ask SignalSpace v3 — planning-loop skills ──────────────
...
def _deterministic_ask(skill: str, packet: dict) -> dict:
    ...
    # ── Pass 22 deterministic answers for the planning loop ─────────
        ...
        if sts is None:
            answer = ("No saved plan yet — SignalSpace needs a spending "
        ...
        answer = (
            ...
            "All are read-only suggestions — SignalSpace never schedules "
            "them automatically."
        )
        bullets = rs
    else:
...
def ask_ledger(question: str, conn: sqlite3.Connection,
    ...
    if skill is None:
        ...
        return {
            "skill":        "unsupported",
            "answer":       ("That question is outside what SignalSpace can answer from your "
                             "local data. SignalSpace only explains what's already in your "
...
def _deterministic_reduce(packet: dict) -> dict:
    ...
    if not subs and not ctrl:
        return {
            ...
            "first_move": "Import a few months of statements so SignalSpace can spot recurring "
...
def ai_features_status() -> list[dict]:
    """A map of every AI-powered surface in SignalSpace, for Settings to render."""
    ready, reason = ai_is_ready()
    ai = get_ai_settings()
