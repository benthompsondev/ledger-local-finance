"""SQLite-safe backup and restore helpers for Ledger's local database."""
...
class BackupValidationError(ValueError):
    """Raised when a selected file is not a usable Ledger database."""
...
def validate_database(path: Path, *, allow_legacy: bool = False) -> dict:
    """Validate SQLite integrity and the minimum Ledger schema read-only."""
    ...
    try:
        ...
        if allow_legacy and not (tables & _LEGACY_TABLES):
            raise BackupValidationError(
                "This SQLite file does not contain a recognizable Ledger table."
            )
...
def restore_database(candidate: Path, *, db_path: Optional[Path] = None) -> dict:
    ...
    if selected == live:
        raise BackupValidationError(
            "Choose a backup file, not the active Ledger database."
        )
