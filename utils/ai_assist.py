...
Northstar's whole point is that financial data stays on the machine. This
...
_RETIRED_MINIMAX_MODELS = frozenset({
    # Northstar shipped this MiniMax 01-era default before the native AI
...
def _repair_retired_minimax_model(conn, stored: dict[str, str]) -> str:
    """Replace only Northstar's known retired MiniMax default.

    Existing settings are normally user-owned and must survive an upgrade.
    This one value is different: Northstar itself shipped it, MiniMax now
...
def _base_response_error(answer: dict) -> Optional[AiProviderError]:
    ...
    if code in {1000, 1001, 1002, 1024, 1033}:
        return AiProviderError(
            "MiniMax is temporarily unavailable. Northstar will retry once.",
            kind="transient",
            retryable=True,
        )
    ...
    return AiProviderError(
        "MiniMax returned an error Northstar could not classify.",
        kind="provider_error",
    )
...
def _http_provider_error(exc, *, host: str) -> AiProviderError:
    ...
    if code == 429:
        return AiProviderError(
            ...
            "Northstar will retry once.",
            kind="rate_limit",
            retryable=True,
        )
    if code >= 500:
        return AiProviderError(
            "The AI provider is temporarily unavailable. Northstar will "
            "retry once.",
            kind="transient",
            retryable=True,
        )
    if code == 400 and host == "api.minimax.io":
        return AiProviderError(
            "MiniMax rejected the selected model or request. Northstar's "
...
def _perform(request, *, settings: dict, host: str, timeout: int,
    ...
    try:
        ...
        raise AiProviderError(
            "The AI provider returned a response Northstar could not read.",
            kind="unreadable",
        ) from None
    ...
    try:
        ...
        if "NORTHSTAR_CONNECTION_OK" not in result.text.upper():
            raise ValueError(
                ...
                "complete Northstar's connection check."
            )
        return {
            "ok": True,
            "message": (
...
def ask(question: str, conn: Optional[sqlite3.Connection] = None,
    ...
    try:
        ...
        grounding_note = (
            "Dollar and percentage figures were matched to Northstar's local "
        ...
        if _unsupported_figures(text, payload):
            ...
            grounding_note = (
                "Northstar removed suggested financial targets that were not "
