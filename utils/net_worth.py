...
the user can see accounts Northstar cannot.
...
# The currency the readings are kept in. Northstar has never invented an
    ...
    try:
        ...
        for balance in get_account_balances(conn=c, latest_only=True):
            ...
            if currency != LOCAL_CURRENCY:
                # Northstar has no exchange rate and invents one nowhere
    ...
    try:
        ...
        if not entries:
            return {
                "has_entries": False,
                "reason": (
                    "Record what you are worth this month and Northstar will "
