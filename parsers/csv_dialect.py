...
def detect_date_format(values: list[str]) -> tuple[str, list[str], list[str]]:
    ...
    if not cells:
        return "", [
            "SignalSpace could not read any of the dates in this file."
        ], []
    ...
    for day_fmt, month_fmt, _sep in _SLASH_PAIRS:
        ...
        if only_day and only_month:
            return "", [
                ...
                f"'{only_day[0]}' can only be day-first. SignalSpace will not "
    ...
    return "", [
        "SignalSpace could not recognise the date format in this file."
    ], []
...
def detect_decimal_separator(
    ...
    if comma_decimal and dot_decimal:
        return ".", [], [
            ...
            "amounts (10.00). SignalSpace will not guess which number "
            "convention applies."
        ]
    if comma_decimal:
    ...
    if len(grouping) > 1:
        return ".", [], [
            "This file groups thousands with both dots and commas. SignalSpace "
    ...
    if ambiguous:
        ...
        return ".", [], [
            f"SignalSpace cannot tell whether the {name} in '{example}' is a "
