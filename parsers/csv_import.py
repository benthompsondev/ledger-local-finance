...
def read_csv_text(
    ...
    raise ValueError(
        "Northstar could not read this CSV's text encoding. Export it as "
        ...
        if not has_header:
            ...
            if chosen is not None and chosen.positional_roles:
                ...
                dialect.problems.append(
                    ...
                    "bank Northstar knows. Confirm which column is which, or "
                    "choose your bank."
                )
                return _empty_result(
...
def _row_width_problems(header: list[str], body: list[list[str]],
    ...
    if long_rows:
        problems.append(
            ...
            "value after it. Northstar will not guess which value is the "
            "amount."
        )
    if short_rows:
        problems.append(
...
def _column_role_problems(
    ...
    if duplicate_names:
        problems.append(
            ...
            + ". Northstar cannot tell those columns apart."
        )
...
def _parse_rows(filepath, statement_period, dialect, rows, col_map, *,
    ...
    try:
        ...
        if not signed_amount_populated and not split_amount_populated:
            dialect.problems.append(
                "Northstar could not find any transaction amounts in this file."
            )
            return _empty_result(
                filepath, statement_period, dialect, dialect.problems,
                needs_mapping=True,
        ...
        if not date_field:
            dialect.problems.append(
                "Northstar could not find a date column in this file."
            )
            return _empty_result(
                filepath, statement_period, dialect, dialect.problems,
                needs_mapping=True,
        ...
        # Money in two currencies cannot be added up. Northstar has no
        ...
        if len(currencies) > 1:
            dialect.problems.append(
                ...
                + ". Northstar cannot convert between currencies, so these "
    ...
    if unreadable_dates:
        ...
        dialect.problems.append(
            f"{len(unreadable_dates)} row(s) have a date Northstar cannot "
    ...
    if unreadable_amounts:
        ...
        dialect.problems.append(
            f"{len(unreadable_amounts)} row(s) have an amount Northstar "
    ...
    if contradictory_amounts:
        ...
        dialect.problems.append(
            ...
            f"a credit value ({shown}). Northstar will not choose one and "
            "discard the other."
        )
    ...
    if errors and not unreadable_dates:
        dialect.problems.append(
            f"{len(errors)} row(s) could not be read. Northstar does not "
...
def _redundant_amount_layout_problem(
    ...
    for index, row in enumerate(mapped_rows, start=1):
        ...
        if signed is None or debit is None or credit is None:
            return (
                "Northstar could not reconcile the repeated amount columns "
                f"on data row {index}."
            )
    ...
    for column in ("tx_type", "type", "direction"):
        ...
        if unknown:
            ...
            return "", (
                ...
                f"the unknown value '{shown}'. Northstar will not guess "
